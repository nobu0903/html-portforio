...
class Game:
    ...
    def __init__(self, player, monster_group, power_up_group):
        ...
        self.power_up_group = power_up_group
    ...
    def check_collisions(self):
        ...
        collided_power_up = pygame.sprite.spritecollideany(self.player, self.power_up_group)

        if collided_power_up:
            self.player.activate_invincibility()
            collided_power_up.kill()

        # Check for collision between a player and monsters
        collided_monster = pygame.sprite.spritecollideany(self.player, self.monster_group)

        if collided_monster:
            if self.player.invincible:
                # If the player is invincible, remove the monster
...
class Player(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.invincible_sound = pygame.mixer.Sound("shikanoko.mp3")
...
class Monster(pygame.sprite.Sprite):
    """A class to create the monster objects"""
...
class PowerUp(pygame.sprite.Sprite):
    """A class to create power-up objects"""
    ...
    def update(self):
        """Update the power-up (if needed)"""
        pass

# Load power-up image
power_up_image = pygame.image.load("apple.png")

# Create a power-up group
power_up_group = pygame.sprite.Group()

def place_power_up():
    """Place a power-up on the screen"""
    x = random.randint(0, WINDOW_WIDTH - 64)
    y = random.randint(100, WINDOW_HEIGHT - 164)
    power_up = PowerUp(x, y, power_up_image)
...
my_power_up_group = power_up_group
...
# Create Game object
my_game = Game(my_player, my_monster_group, my_power_up_group)
...
while running:
    ...
    power_up_group.draw(display_surface)
